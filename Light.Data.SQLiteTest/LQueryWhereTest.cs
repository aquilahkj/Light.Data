...
namespace Light.Data.SQLiteTest
{
	...
	public class LQueryWhereTest:BaseTest
	{
		...
		public void TestCase_QuerySingleParam_Double ()
		{
			...
			Assert.AreEqual (listEx.Count, listAc.Count);
			Assert.IsTrue (listAc.TrueForAll (x => x.HotRate == (double)1.10f));
			...
			Assert.AreEqual (listEx.Count, listAc.Count);
			Assert.IsTrue (listAc.TrueForAll (x => x.HotRate > (double)1.05f && x.HotRate < (double)1.15f));
		...
		*/
		...
		[Test ()]
		public void TestCase_QueryInArray_Int ()
		{
			List<TeUser> list = InitialUserTable (21);
			List<TeUser> listEx;
			List<TeUser> listAc;
		...
		[Test ()]
		public void TestCase_QueryInArray_Double ()
		{
			List<TeUser> list = InitialUserTable (21);
			List<TeUser> listEx;
			List<TeUser> listAc;
			...
			double[] arrayxf = new double[]{ 1.03f, 1.05f, 1.07f };
			List<double> listxf = new List<double> (arrayxf);

			listEx = list.FindAll (x => listx.Exists (y => x.HotRate == y));
			listAc = context.LQuery<TeUser> ().Where (TeUser.HotRateField.In (arrayx)).ToList ();
			Assert.AreEqual (listEx.Count, listAc.Count);
			Assert.IsTrue (listAc.TrueForAll (x => listxf.Exists (y => x.HotRate == y)));

			listEx = list.FindAll (x => listx.TrueForAll (y => x.HotRate != y));
			listAc = context.LQuery<TeUser> ().Where (TeUser.HotRateField.NotIn (arrayx)).ToList ();
			Assert.AreEqual (listEx.Count, listAc.Count);
			Assert.IsTrue (listAc.TrueForAll (x => listxf.TrueForAll (y => x.HotRate != y)));
		}

		[Test ()]
		public void TestCase_QueryInArray_DateTime ()
		{
			List<TeUser> list = InitialUserTable (21);
			List<TeUser> listEx;
			List<TeUser> listAc;
