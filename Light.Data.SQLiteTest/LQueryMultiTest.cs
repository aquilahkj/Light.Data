...
namespace Light.Data.SQLiteTest
{
	...
	public class LQueryMultiTest:BaseTest
	{
		...
		public void TestCase_WhereOrderBy ()
		{
			...
			List<TeUser> list3 = context.LQuery<TeUser> ().Where (TeUser.DeleteFlagField.IsTrue()).OrderBy (TeUser.IdField.OrderByAsc ()).ToList ();
			Assert.AreEqual (10, list3.Count);
			for (int i = 1; i < list3.Count; i++) {
				Assert.Less (list3 [i - 1].Id, list3 [i].Id);
			}

			List<TeUser> list4 = context.LQuery<TeUser> ().Where (TeUser.DeleteFlagField.IsTrue()).OrderBy (TeUser.IdField.OrderByDesc ()).ToList ();
