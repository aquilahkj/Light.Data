    ...
    abstract class CommandFactory
    {
        ...
        public virtual IDbCommand CreateSelectSingleFieldCommand(DataFieldInfo fieldinfo, QueryExpression query, OrderExpression order, bool distinct, Region region)
        {
            ...
            if (fieldMapping is PrimitiveFieldMapping || fieldMapping is EnumFieldMapping || fieldMapping is CustomFieldMapping)
            {
                ...
                return CreateSelectBaseCommand(mapping, select, query, order, region);//, distinct);
            }
            else
            {
        ...
        public virtual IDbCommand[] CreateBulkInsertCommand(Array entitys, int batchCount)
        {
            ...
            foreach (object entity in entitys)
            {
                ...
                {
                    //totalSql.Remove(totalSql.Length - 1, 1);
...
///// </summary>
///// <param name="sql">SQL语句</param>
///// <param name="dataParameters">DataParameter参数</param>
///// <returns>SQL参数</returns>
//protected IDataParameter[] TransferSqlStringParams(StringBuilder sql, DataParameter[] dataParameters)
//{
//    if (dataParameters == null || dataParameters.Length == 0)
//    {
//        return new IDataParameter[0];
//    }
//    IDataParameter[] idataParameters = new IDataParameter[dataParameters.Length];
//    int length = dataParameters.Length;
//    for (int i = 0; i < length; i++)
//    {
//        DataParameter dp = dataParameters[i];
//        idataParameters[i] = _database.CreateParameter("P" + i, dp.Value, dp.DbType, dp.Direction);
//        sql.Replace(dataParameters[i].ParameterName, idataParameters[i].ParameterName);
//    }
//    return idataParameters;
//}

//public virtual IDbCommand TransferCommand(string sql, IDataParameter[] dataParameters)
//{
//    IDbCommand command = _database.CreateCommand(sql);
//    command.CommandType = CommandType.Text;
//    foreach (IDataParameter param in dataParameters)
