...
{
	[TestFixture ()]
	public class TT_RelationSingleTest : BaseTest
	{
		[Test ()]
		public void TestCase_Base ()
		{
			...
			list = context.Query<TeUserWithLevel4> ().ToList ();
		...
		public void TestCase_Inherit ()
		{
			...
			list = context.Query<TeUserWithLevel5> ().ToList ();
			Assert.AreEqual (dict.Count, list.Count);
			foreach (KeyValuePair<TeUser, TeUserLevel> kvs in dict) {
		...
		public void TestCase_NoEntity ()
		{
			...
			list = context.Query<TeUserWithLevel6> ().ToList ();
			Assert.AreEqual (dict.Count, list.Count);
			foreach (KeyValuePair<TeUser, TeUserLevel> kvs in dict) {
		...
		public void TestCase_Inherit_Multi ()
		{
			...
			users = context.Query<TeUser> ().ToList ();
			levels = context.Query<TeUserLevel> ().ToList ();
			areas = context.Query<TeAreaInfo> ().ToList ();
			...
			list = context.Query<TeUserWithLeveAndArea> ().ToList ();
		...
		public void TestCase_Inherit_OrderBy ()
		{
			...
			list = context.Query<TeUserWithLevel5> ().OrderByDescending (x => x.Id).ToList ();
		...
		public void TestCase_Where ()
		{
			...
			users = context.Query<TeUser> ().Where (x => x.Id > 2 && x.Id <= 10).ToList ();
			levels = context.Query<TeUserLevel> ().ToList ();
			dict = new Dictionary<TeUser, TeUserLevel> ();
			dict1 = new Dictionary<int, List<TeUserWithLevel5>> ();
			foreach (TeUser user in users) {
			...
			list = context.Query<TeUserWithLevel5> ().Where (x => x.Id > 2 && x.Id <= 10).ToList ();
		...
		[Test ()]
		public void TestCase_Where_Relate ()
		{
			InitialUserTable (50);
			InitialUserLevelTable (6);
			...
			users = context.Query<TeUser> ().Where (x => x.Id > 2 && x.Id <= 10).ToList ();
			levels = context.Query<TeUserLevel> ().Where (x => x.Id > 3).ToList ();
			dict = new Dictionary<TeUser, TeUserLevel> ();
			dict1 = new Dictionary<int, List<TeUserWithLevel5>> ();
			foreach (TeUser user in users) {
				var item = levels.Find (x => x.Id == user.LevelId);
				if (item != null) {
					dict [user] = item;
				}
			}
			list = context.Query<TeUserWithLevel5> ().Where (x => x.Id > 2 && x.Id <= 10 && x.UserLevel.Id > 3).ToList ();
		...
		[Test ()]
		public void TestCase_Where_Sub ()
		{
			InitialUserTable (50);
			InitialUserLevelTable (6);
			...
			list = context.Query<TeUserWithLevel5> ().Where (x => x.UserLevel.Id > 3).ToList ();
