...
{
	[TestFixture ()]
	public class TT_RelationMultiTest : BaseTest
	{
		[Test ()]
		public void TestCase_CollectionAndSingle ()
		{
			...
			list = context.Query<TeUserLevelWithUserRefer> ().ToList ();
		...
		public void TestCase_SingleAndSingle ()
		{
			...
			users = context.Query<TeUser> ().ToList ();
			extends = context.Query<TeUserExtend> ().ToList ();

			list = context.Query<TeUserWithExtendRefer> ().ToList ();
		...
		public void TestCase_SingleAndSingle1 ()
		{
			...
			list = context.Query<TeUserWithExtendRefer1> ().ToList ();
			Assert.AreEqual (users.Count, list.Count);
			foreach (TeUser user in users) {
				TeUserExtend extend = extends.Find (x => x.UserId == user.Id);
		...
		public void TestCase_SingleAndSingle2 ()
		{
			...
			users = context.Query<TeUser> ().ToList ();
			extends = context.Query<TeUserExtend> ().ToList ();
			areaInfos = context.Query<TeAreaInfo> ().ToList ();
			list = context.Query<TeUserWithExtendRefer2> ().ToList ();
		...
		public void TestCase_SingleAndSingle_r1 ()
		{
			...
			List<TeRelateA_BE> relateList = context.Query<TeRelateA_BE> ().ToList ();
